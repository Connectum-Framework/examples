...
The Queues tab shows durable queues created by the adapter — one per consumer group (`${exchange}.${group}`), each bound to every routing key that group subscribes to:
