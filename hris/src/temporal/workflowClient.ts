/**
 * Lazy Temporal client factory for the gateway (OnboardingService) role.
 *
 ...
 * Temporal server: only an actual `OnboardEmployee` that reaches
 * `workflowClient.start` (or a `GetOnboarding` query) touches Temporal.
 *
...
/** The `WorkflowClient` shape (a subset is consumed by `createOnboardingService`). */
